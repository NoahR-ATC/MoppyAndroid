    ...
    protected void processAudioBuffers() {
        ...
        for (int i = 0; i < buffers.length; i++) {
            if (i != CHANNEL_DELAY_LEFT &&
                ...
                i != CHANNEL_DELAY_EFFECT2) { buffers[i].clear(); }
        }
        ...
        synchronized (control_mutex) {
            ...
            for (int i = 0; i < voicestatus.length; i++)
                if (voicestatus[i].active) { voicestatus[i].processControlLogic(); }
            ...
            if (act_registeredMixers != null) {
                if (act_registeredMixers.length == 0) { act_registeredMixers = null; }
            }
