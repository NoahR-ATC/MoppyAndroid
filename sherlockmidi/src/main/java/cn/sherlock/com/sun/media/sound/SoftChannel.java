...
*/
        ...
        if (controller < 120) {
            for (int i = 0; i < voices.length; i++)
                if (voices[i].active) { voices[i].controlChange(controller, -1); }
        }
        else if (controller == 120) {
            for (int i = 0; i < voices.length; i++)
                if (voices[i].active) { voices[i].rpnChange(1, -1); }
        }
        else if (controller == 121) {
            for (int i = 0; i < voices.length; i++)
                if (voices[i].active) { voices[i].rpnChange(2, -1); }
        }
    ...
    public void controlChange(int controller, int value) {
        ...
        synchronized (control_mutex) {
            ...
            for (int i = 0; i < voices.length; i++)
                if (voices[i].active) { voices[i].controlChange(controller, value); }
    ...
    public void setPitchBend(int bend) {
        ...
        synchronized (control_mutex) {
            ...
            pitchbend = bend;
            for (int i = 0; i < voices.length; i++)
                if (voices[i].active) { voices[i].setPitchBend(bend); }
        }
    }
    ...
    public void nrpnChange(int controller, int value) {
        ...
        for (int i = 0; i < voices.length; i++)
            if (voices[i].active) { voices[i].nrpnChange(controller, val_i[0]); }
    ...
    public void rpnChange(int controller, int value) {
        ...
        for (int i = 0; i < voices.length; i++)
            if (voices[i].active) { voices[i].rpnChange(controller, val_i[0]); }
    }
    ...
    public void allSoundOff() {
        ...
        synchronized (control_mutex) {
            for (int i = 0; i < voices.length; i++)
                if (voices[i].on && voices[i].channel == channel) { voices[i].soundOff(); }
        }
    }
    ...
    public void setMute(boolean mute) {
        ...
        synchronized (control_mutex) {
            this.mute = mute;
            for (int i = 0; i < voices.length; i++)
                if (voices[i].active && voices[i].channel == channel) { voices[i].setMute(mute); }
        }
    }
